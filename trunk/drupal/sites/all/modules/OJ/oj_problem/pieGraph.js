//http://hi.baidu.com/zzguang/blog/item/2ebcae187633090035fa41dc.html
//3D Pie 类封装
//2007-06-07 updated by Dragoon
/* setPie(container,title,arrName,arrData,arrColor,sizeX,sizeY)
** 饼图设置函数：初始化饼图数据并绘制
** container(String)为放置饼图的容器名称，推荐DIV对象
** title(String)为饼图标题
** arrName(String)为饼图分区名称
** arrColor(String) 颜色字符串
** arrData(number)为饼图分区数据，为绝对数值，本函数将以绝对数值的和为1计算各数据的百分比
**(注意：分区名称，数据和颜色字符串数组的元素个数应一致，按顺序存放)
** sizeX(number)为宽度
** sizeY(number)为高度
*/
onit = true;
...
function Pie3D(pContainer, pwidth, pheight, pcaption) {
    ...
    this.Text3d = "<v:shapetype id='3dtxt' coordsize='21600,21600' o:spt='136' adj='10800' path='m@7,l@8,m@5,21600l@6,21600e'>" + "<v:path textpathok='t' o:connecttype='custom' o:connectlocs='@9,0;@10,10800;@11,21600;@12,10800' o:connectangles='270,180,90,0'/>" + "<v:textpath on='t' fitshape='t'/>" + "<o:lock v:ext='edit' text='t' shapetype='t'/>" + "</v:shapetype>";
    this.draw = function() {
        this.init();
        this.drawCanvas();
        this.drawCake();
...
Pie3D.prototype.drawLegend = function(canvas) {
    ...
    captiontb.innerHTML = "<table width='100%' border='0' align='center' cellspacing='0'><tr><td align='center' valign='middle'>" + "<div style='font-size:14pt; '><B>" + this.caption + "</B></div>" + "</td></tr></table>";
    ...
    fillStyle = "<v:fill rotate='t' angle='-175' focus='100%' type='gradient'/>" + "<v:shadow on='t' type='single' color='silver' offset='1pt,1pt'/>";
    ...
    fillStyle = "<v:TextBox inset='2pt,0pt,0pt,0pt' style='font-size:10pt;'>" + "<div align='left'><font color='#000'>共:" + this.vTotal + "</font></div></v:TextBox>";
...
Pie3D.prototype.drawCake = function() {
    ...
    for (var i = 0; i < this.Cakes.length; i++) {
        ...
        rotates = k1 + k2;
        if (rotates >= 360) rotates = rotates - 360;
        ...
        cakepie.innerHTML = "<v:shape id='cake" + (i + 1) + "' type='#Cake_3D' title='" + this.Cakes[i][0] + ":" + xRound(this.Cakes[i][1], 2) + "比例:" + xRound(this.Percent[i] * 100, 2) + "%'" + " style='position:absolute;left:" + (_left + Height / 8) + "px;top:" + (_top + Height / 14) + "px;WIDTH:" + Height + "px;HEIGHT:" + Height + "px;rotation:" + rotates + ";z-index:" + zIndex + "' adj='" + adjs + ",0'" + " fillcolor='" + this.Cakes[i][2] + "' onmouseover='moveup(cake" + (i + 1) + "," + (_top + Height / 14) + ",txt" + (i + 1) + ",rec" + (i + 1) + ")'; onmouseout='movedown(cake" + (i + 1) + "," + (_top + Height / 14) + ",txt" + (i + 1) + ",rec" + (i + 1) + ");'>" + "<v:fill opacity='60293f' color2='fill lighten(120)' o:opacity2='60293f' rotate='t' angle='-135' method='linear sigma' focus='100%' type='gradient'/>" + "<o:extrusion v:ext='view' on='t' backdepth='16' rotationangle='60' viewpoint='0,0'viewpointorigin='0,0' skewamt='0' lightposition='-50000,-50000' lightposition2='50000'/>" + "</v:shape>";
        cakepie.innerHTML += "<v:shape id='txt" + (i + 1) + "' type='#3dtxt' style='position:absolute;left:" + txt_x + "px;top:" + txt_y + "px;z-index:20;display:none;width:50; height:12;'" + " fillcolor='red' onmouseover='ontxt(cake" + (i + 1) + "," + (_top + Height / 14) + ",txt" + (i + 1) + ",rec" + (i + 1) + ")'>" + "<v:fill opacity='60293f' color2='fill lighten(120)' o:opacity2='60293f' rotate='t' angle='-135' method='linear sigma' focus='100%' type='gradient'/>" + "<v:textpath style='v-text-kern:t' trim='t' fitpath='t' string='" + this.Cakes[i][0] + ":" + xRound(this.Percent[i] * 100, 2) + "%'/> " +
        ...
        k1 = k1 + k2 * 2;
        if (k1 >= 360) k1 = k1 - 360;
        if (k1 > 180) zIndex = zIndex + 1;
        else zIndex = zIndex - 1;
    }
}
/* 
...
** sizeY(number)为高度
*/
function setPie(container, title, arrName, arrData, arrColor, sizeX, sizeY) {
    //如名称和数据数组长度不一，应报错返回
    if (arrName.length != arrData.length || arrName.length != arrColor.length) {
        alert("饼图分区名称、数据和颜色个数不一致！请检查！");
        return;
    }
    var pieGraph = new Pie3D(container, sizeX, sizeY, title);
    for (var i = 0; i < arrData.length; i++) {
        var partition = new Array(arrName[i], arrData[i], arrColor[i]);
        pieGraph.Cakes[i] = partition;
    }
    pieGraph.draw();
}